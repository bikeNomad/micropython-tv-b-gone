        ...
        if pulse_match and pause_match:
            ...
            if header_pulse is not None and header_pause is not None and hp > 0 and hpa > 0:
                ...
                if not (header_pulse >= hp_min and header_pulse <= hp_max and 
                    ...
                    header_pulse_variation = calculate_variation(header_pulse, hp)
                    header_pause_variation = calculate_variation(header_pause, hpa)
            ...
            if header_match:
                ...
                if header_pulse_variation is not None and header_pause_variation is not None:
                    avg_variation = (abs(header_pulse_variation) + abs(header_pause_variation)) / 2
...
def analyze_signal(csv_file):
    ...
    for i in range(len(packet_starts)):
            ...
            if len(rep_times) >= 4:
                ...
                for k in range(2, len(rep_times)-1, 2):
                    if k+1 < len(rep_times) and rep_states[k] == 0 and rep_states[k+1] == 1:
                        other_pulses.append((rep_times[k+1] - rep_times[k]) * 1_000_000)
        ...
        # Bit statistics (including all bits)
        if all_bits:
            # Only include valid pauses (not None and not 0)
            all_pulses = [bit[0] * 1_000_000 for bit in all_bits]  # Convert to microseconds
            all_pauses = [bit[1] * 1_000_000 for bit in all_bits if bit[1] is not None]  # Convert to microseconds
            
            min_pulse = min(all_pulses) if all_pulses else 'N/A'
            max_pulse = max(all_pulses) if all_pulses else 'N/A'
            min_pause = min(all_pauses) if all_pauses else 'N/A'
            max_pause = max(all_pauses) if all_pauses else 'N/A'
            ...
            if isinstance(min_pulse, float) and isinstance(max_pulse, float) and \
               isinstance(min_pause, float) and isinstance(max_pause, float):
                possible_protocols, protocol_variations = match_protocol(
                    min_pulse, max_pulse, min_pause, max_pause, header_pulse, header_pause
                )
        else:
        ...
        results.append({
            ...
            'min_pause_us': min_pause,
            'max_pause_us': max_pause,
...
def main():
    ...
    for packet in display_results:
        ...
        print(f"  Bit timing: Pulse (min={format_us(packet['min_pulse_us'])}, "
    ...
    if args.output:
        ...
        with open(args.output, 'w') as f:
            ...
            f.write("# Format: List of (packet_index, start_time, protocol_name, [pulse_duration, pause_duration, ...])\n")
            ...
            for packet_idx, start_time, protocol_name, durations in packet_timings:
                f.write(f"    # Packet {packet_idx} (start time: {start_time:.6f}s) - {protocol_name} protocol\n")
                f.write(f"    {tuple(durations)},\n")
            
            f.write("]\n")
        
        print(f"Wrote {len(packet_timings)} recognized packets to {args.output}")
