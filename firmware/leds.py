...
rgb_pwr = Pin(RGB_LED_POWER_PIN, Pin.OUT,
              value=1, hold=False) if RGB_LED_POWER_PIN is not None else None

rgb_led = None
if RGB_LED_PIN is not None:
    Pin(RGB_LED_PIN, Pin.OUT).init(mode=Pin.OUT, value=0, hold=False)
...
def sleep_leds():
    ...
    if rgb_led is not None:
        rgb_led[0] = BLACK
        rgb_led.write()
        Pin(RGB_LED_PIN, Pin.OUT).init(mode=Pin.OUT, value=0, hold=True)

    if user_led is not None:
        user_led.init(mode=Pin.OUT, value=not USER_LED_ACTIVE_LEVEL, hold=True)

    if rgb_pwr is not None:
        # Turn off RGB LED power
        rgb_pwr.init(mode=Pin.OUT, value=0, hold=True)
